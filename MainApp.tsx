...
const MainApp: React.FC<MainAppProps> = ({ user, onLogout, onProfileUpdate, theme, setTheme }) => {
  ...
  useEffect(() => {
    ...
    const friendRequestsChannel = subscriptions.subscribeToFriendRequests(user.id, async (payload) => {
      const { eventType, new: newRecord, old: oldRecord } = payload;

      if (eventType === 'INSERT') {
        const newRequest = { id: newRecord.id, fromUserId: newRecord.from_user_id, toUserId: newRecord.to_user_id };
        setFriendRequests(prev => [...prev, newRequest]);
      } else if (eventType === 'DELETE') {
        setFriendRequests(prev => prev.filter(req => req.id !== oldRecord.id));
      }
    });
    ...
    const channel = subscriptions.subscribeToSessions(async (payload) => {
      ...
      switch (eventType) {
        case 'INSERT': {
          ...
          // Only show toast if creator is a friend OR it's a private session visible to user's tags
          if (newSession.creator_id !== user.id) {
            const isFriend = friendsRef.current.some(f => f.id === newSession.creator_id);
            const isInvitedPrivate = newSession.privacy === 'private' &&
              newSession.visibleToTags?.some(tagId => tagsRef.current.some(t => t.id === tagId));

            if (isFriend || isInvitedPrivate) {
              addToast(`New session: "${newSession.title}"`, 'info');
            }
          }
          break;
        }
  ...
  const handleSocialActions = useMemo(() => ({
    ...
    handleAcceptRequest: async (fromUserId: string) => {
      let request = friendRequests.find(req => req.fromUserId === fromUserId && req.toUserId === user.id);

      // If not found in state, try fetching from database (handles race conditions)
      if (!request) {
        const { data: allRequests } = await supabaseService.fetchFriendRequests(user.id);
        request = allRequests?.find(req => req.fromUserId === fromUserId && req.toUserId === user.id);
  ...
  const handleSaveTag = useCallback(async (tagData: Omit<Tag, 'id' | 'memberIds' | 'creator_id'>) => {
    if (editingTag) {
      const { data, error } = await supabaseService.updateTag(editingTag.id, tagData); if (error || !data) { addToast("Could not update tag.", "error"); } else { setTags(prev => prev.map(t => t.id === editingTag.id ? data[0] : t)); addToast("Tag updated!", "success"); }
